  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (
        ...
        shouldShowErrorToast(fromCurrency, toCurrency, error)
      ) {
        const errorMessage =
          "This currency pair is currently inactive or not supported. Please select a different pair.";
        toast({
          variant: "destructive",
...
export async function getEstimatedExchangeAmount(
  ...
  if (!res.ok) {
    ...
    if (
      shouldShowErrorToast(fromCurrency, toCurrency, error || "general_error")
    ) {
      toast({
        variant: "destructive",
...
export async function getUserAddresses(
  ...
  try {
    ...
    const response = await fetch(
      `${CHANGENOW_API_BASE}/addresses-by-name?name=${encodeURIComponent(
        name
      )}`,
      { headers }
    );
...
export async function createFiatTransaction(
  ...
  try {
    const response = await fetch(`${CHANGENOW_API_BASE}/fiat-transaction`, {
      method: "POST",
      headers: {
...
export async function getFiatHealthCheck(): Promise<FiatHealthCheckResponse | null> {
  try {
    const response = await fetch(`${CHANGENOW_API_BASE}/fiat-status`, {
      headers: {
        "x-changenow-api-key": CHANGENOW_API_KEY,
      },
    });
...
export async function getFiatCurrencies(): Promise<FiatCurrency[] | null> {
  try {
    const response = await fetch(`${CHANGENOW_API_BASE}/fiat-currencies/fiat`, {
      // headers: {
      //     'x-changenow-api-key': CHANGENOW_API_KEY,
      // },
    });
...
export async function refundExchange(
  ...
  try {
    const response = await fetch(`${CHANGENOW_API_BASE}/exchange/refund`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-changenow-api-key": CHANGENOW_API_KEY,
...
export async function continueExchange(
  ...
  try {
    const response = await fetch(`${CHANGENOW_API_BASE}/exchange/continue`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-changenow-api-key": CHANGENOW_API_KEY,
